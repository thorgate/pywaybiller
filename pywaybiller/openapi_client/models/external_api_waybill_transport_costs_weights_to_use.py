...
EXTERNALAPIWAYBILLTRANSPORTCOSTSWEIGHTSTOUSE_ONE_OF_SCHEMAS = [
    "BlankEnum",
    "WeightsToUseEnum",
]
...
class ExternalAPIWaybillTransportCostsWeightsToUse(BaseModel):
    ...
    oneof_schema_1_validator: Optional[WeightsToUseEnum] = None
    # data type: BlankEnum
    oneof_schema_2_validator: Optional[BlankEnum] = None
    actual_instance: Optional[Union[BlankEnum, WeightsToUseEnum]] = None
    one_of_schemas: Set[str] = {"BlankEnum", "WeightsToUseEnum"}
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = ExternalAPIWaybillTransportCostsWeightsToUse.model_construct()
        error_messages = []
        match = 0
        # validate data type: WeightsToUseEnum
        if not isinstance(v, WeightsToUseEnum):
            error_messages.append(
                f"Error! Input type `{type(v)}` is not `WeightsToUseEnum`"
            )
        else:
            match += 1
        ...
        if match > 1:
            # more than 1 match
            raise ValueError(
                "Multiple matches found when setting `actual_instance` in ExternalAPIWaybillTransportCostsWeightsToUse with oneOf schemas: BlankEnum, WeightsToUseEnum. Details: "
                + ", ".join(error_messages)
            )
        elif match == 0:
            # no match
            raise ValueError(
                "No match found when setting `actual_instance` in ExternalAPIWaybillTransportCostsWeightsToUse with oneOf schemas: BlankEnum, WeightsToUseEnum. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into WeightsToUseEnum
        try:
            instance.actual_instance = WeightsToUseEnum.from_json(json_str)
            match += 1
        ...
        if match > 1:
            # more than 1 match
            raise ValueError(
                "Multiple matches found when deserializing the JSON string into ExternalAPIWaybillTransportCostsWeightsToUse with oneOf schemas: BlankEnum, WeightsToUseEnum. Details: "
                + ", ".join(error_messages)
            )
        elif match == 0:
            # no match
            raise ValueError(
                "No match found when deserializing the JSON string into ExternalAPIWaybillTransportCostsWeightsToUse with oneOf schemas: BlankEnum, WeightsToUseEnum. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], BlankEnum, WeightsToUseEnum]]:
