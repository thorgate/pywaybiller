...
EXTERNALAPIVEHICLELOCATIONDATAVEHICLELOCATIONDATAINNERVALUE_ONE_OF_SCHEMAS = [
    "datetime",
    "float",
]


class ExternalAPIVehicleLocationDataVehicleLocationDataInnerValue(BaseModel):
    """
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = ExternalAPIVehicleLocationDataVehicleLocationDataInnerValue.model_construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError(
                "Multiple matches found when setting `actual_instance` in ExternalAPIVehicleLocationDataVehicleLocationDataInnerValue with oneOf schemas: datetime, float. Details: "
                + ", ".join(error_messages)
            )
        elif match == 0:
            # no match
            raise ValueError(
                "No match found when setting `actual_instance` in ExternalAPIVehicleLocationDataVehicleLocationDataInnerValue with oneOf schemas: datetime, float. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError(
                "Multiple matches found when deserializing the JSON string into ExternalAPIVehicleLocationDataVehicleLocationDataInnerValue with oneOf schemas: datetime, float. Details: "
                + ", ".join(error_messages)
            )
        elif match == 0:
            # no match
            raise ValueError(
                "No match found when deserializing the JSON string into ExternalAPIVehicleLocationDataVehicleLocationDataInnerValue with oneOf schemas: datetime, float. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance
