...
class ExternalAPIWaybillRefuse(BaseModel):
    """
    ...
    destination_raw_id: Optional[StrictStr] = Field(
        None, description="The ID of the new destination."
    )
    __properties = ["reason", "destination_raw_id"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ExternalAPIWaybillRefuse:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> ExternalAPIWaybillRefuse:
        """Create an instance of ExternalAPIWaybillRefuse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ExternalAPIWaybillRefuse.parse_obj(obj)

        _obj = ExternalAPIWaybillRefuse.parse_obj(
            {
