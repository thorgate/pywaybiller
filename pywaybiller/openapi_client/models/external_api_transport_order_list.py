...
from pydantic import BaseModel, StrictInt, conlist, constr
...
class ExternalAPITransportOrderList(BaseModel):
    ...
    transport_order_id: Optional[constr(strict=True, min_length=1)] = None
    waybills_ids: Optional[conlist(StrictInt, unique_items=True)] = None
    raw_data: Optional[ExternalAPITransportOrderRawDataList] = None
    __properties = ["transport_order_id", "waybills_ids", "raw_data"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ExternalAPITransportOrderList:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> ExternalAPITransportOrderList:
        """Create an instance of ExternalAPITransportOrderList from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ExternalAPITransportOrderList.parse_obj(obj)

        _obj = ExternalAPITransportOrderList.parse_obj(
            {
