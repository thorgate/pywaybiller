...
class ExternalAPIWaybillDispatchedAmounts(BaseModel):
    """
    ExternalAPIWaybillDispatchedAmounts
    """

    dispatched_amounts: conlist(ExternalAPIWaybillRowDispatchedAmount) = Field(
        ..., description="Dispatched amounts"
    )
    __properties = ["dispatched_amounts"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ExternalAPIWaybillDispatchedAmounts:
    ...
    def to_dict(self):
        ...
        if self.dispatched_amounts:
            for _item in self.dispatched_amounts:
                if _item:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> ExternalAPIWaybillDispatchedAmounts:
        """Create an instance of ExternalAPIWaybillDispatchedAmounts from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ExternalAPIWaybillDispatchedAmounts.parse_obj(obj)

        _obj = ExternalAPIWaybillDispatchedAmounts.parse_obj(
            {
                "dispatched_amounts": [
                    ExternalAPIWaybillRowDispatchedAmount.from_dict(_item)
                    for _item in obj.get("dispatched_amounts")
                ]
