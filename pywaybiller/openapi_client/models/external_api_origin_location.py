...
class ExternalAPIOriginLocation(BaseModel):
    """
    ExternalAPIOriginLocation
    """  # noqa: E501

    id: StrictInt = Field(description="Unique identifier of the origin's location")
    address: Annotated[str, Field(strict=True, max_length=255)] = Field(
        description="Physical address of the origin"
    )
    lat: Union[StrictFloat, StrictInt] = Field(
        description="Latitude coordinate of the origin's location"
    )
    lng: Union[StrictFloat, StrictInt] = Field(
        description="Longitude coordinate of the origin's location"
    )
    gmaps_link: StrictStr = Field(
        description="Direct Google Maps link to the location for navigation purposes"
    )
    __properties: ClassVar[List[str]] = ["id", "address", "lat", "lng", "gmaps_link"]
