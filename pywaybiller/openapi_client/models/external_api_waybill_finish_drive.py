...
class ExternalAPIWaybillFinishDrive(BaseModel):
    """
    ExternalAPIWaybillFinishDrive
    """

    kilometres: conint(strict=True, le=999, ge=1) = Field(
        ..., description="Total mileage driven (in kilometres)."
    )
    __properties = ["kilometres"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ExternalAPIWaybillFinishDrive:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> ExternalAPIWaybillFinishDrive:
        """Create an instance of ExternalAPIWaybillFinishDrive from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ExternalAPIWaybillFinishDrive.parse_obj(obj)

        _obj = ExternalAPIWaybillFinishDrive.parse_obj(
            {"kilometres": obj.get("kilometres")}
        )
        return _obj
